...
const A4Page: React.FC<{ children: React.ReactNode; pageNumber: number; totalPages: number }> = ({ children, pageNumber, totalPages }) => (
  <div className="page-a4 relative flex flex-col font-serif antialiased text-black bg-white shadow-2xl print:shadow-none overflow-hidden p-[20mm]">
    <div className="border border-black p-4 mb-6 flex justify-between items-center shrink-0">
    ...
    <div className="flex-1 text-[11px] leading-[1.4] text-justify space-y-4 overflow-hidden">
      {children}
    </div>
  </div>
);
    ...
    <div className="bg-slate-200/50 min-h-screen pb-12 print:bg-white print:p-0">
      ...
      <div ref={contentRef} className="contract-container">
        ...
        <A4Page pageNumber={1} totalPages={totalPages}>
          <p className="text-center font-bold mb-6">CONTRATO DE PRESTAÇÃO DE SERVIÇOS PARA “EXAMES OCUPACIONAIS”</p>
          ...
          <p className="font-bold uppercase border-b border-black pb-1">CLÁUSULA PRIMEIRA – DO OBJETO</p>
          ...
          <p className="font-bold uppercase border-b border-black pb-1 mt-4">CLÁUSULA SEGUNDA – DO LOCAL DA PRESTAÇÃO DOS SERVIÇOS</p>
          ...
          <p className="font-bold uppercase border-b border-black pb-1 mt-4">CLÁUSULA TERCEIRA – DO SISTEMA DE INFORMAÇÃO</p>
        ...
        <A4Page pageNumber={2} totalPages={totalPages}>
          <p className="font-bold uppercase border-b border-black pb-1">CLÁUSULA QUARTA – DO PREÇO E FORMA DE PAGAMENTO</p>
          ...
          <p className="font-bold mt-4">PARÁGRAFO PRIMEIRO:</p>
          ...
          <p className="font-bold mt-4">PARÁGRAFO SEGUNDO:</p>
          ...
          <p className="font-bold uppercase border-b border-black pb-1 mt-6">CLÁUSULA QUINTA - DO PRAZO DE VIGÊNCIA</p>
          ...
          <p className="font-bold uppercase border-b border-black pb-1 mt-6">CLÁUSULA OITAVA – DAS DISPOSIÇÕES GERAIS</p>
          ...
          <p className="mt-12">E, por estarem assim, justos e acordados, assinam o presente Contrato em duas (2) vias em seu anverso, de igual teor e forma, perante duas (2) testemunhas identificadas.</p>
          
          <p className="mt-8 text-center font-bold">Ponta Grossa, PR, {currentDate}.</p>
          
          <div className="mt-16 grid grid-cols-2 gap-12 text-center text-[10px]">
            <div className="space-y-1">
              <div className="border-t border-black pt-2 uppercase font-bold">{companyName}</div>
              <div className="font-bold italic">CONTRATANTE</div>
            </div>
            <div className="space-y-1">
              <div className="border-t border-black pt-2 uppercase font-bold">{contractData.unidadeAtendimento}</div>
              <div className="font-bold italic">CONTRATADA</div>
            </div>
            <div className="space-y-1 mt-4">
              <div className="border-t border-black pt-2">Testemunha 1: Rafaela Lima Cordeiro</div>
              <div className="text-[8px]">CPF: 074.363.239-70</div>
            </div>
            <div className="space-y-1 mt-4">
              <div className="border-t border-black pt-2">Testemunha 2: Samella L. S. Bauer</div>
              <div className="text-[8px]">CPF: 099.466.159-24</div>
            </div>
          </div>
        </A4Page>
        ...
        {selectedUnits.map((unitKey, unitIdx) => (
          <A4Page key={unitKey} pageNumber={3 + unitIdx} totalPages={totalPages}>
            <div className="text-center mb-6">
              <h3 className="text-sm font-bold uppercase underline">ANEXO - TABELA DE VALORES EXAMES | {unitKey.replace('Unidade Reque ', '').toUpperCase()}</h3>
            </div>
              ...
              <table className="w-full text-[9px] border-collapse text-left">
                <thead>
                  <tr className="bg-slate-100 border-b border-black font-bold uppercase text-[8px]">
                    <th className="p-2 border-r border-black w-1/4">TIPO DE EXAME</th>
                    <th className="p-2 border-r border-black w-1/2">NOME DO EXAME</th>
                ...
                <tbody className="divide-y divide-slate-200">
                  {(unitExamsMap[unitKey] || []).map((exam, idx) => (
                    <tr key={idx} className="border-b border-slate-100">
                      <td className="p-2 border-r border-black uppercase font-bold text-slate-600">{exam.category}</td>
                      <td className="p-2 border-r border-black uppercase">{exam.name}</td>
            ...
            <div className="mt-6 p-3 bg-slate-50 border border-slate-200 italic text-[9px]">
